...
static dl_matrix3du_t *image_motion = NULL;
static dl_matrix3du_t *image_rainbow =  NULL;  
static MotionEstContext *me_c = NULL;
...
dl_matrix3du_t *rainbow(MotionVector16_t *vec, float max, int w, int h)
{
    ...
    for(int i = 0; i < len; i++, pvec++) {
        ...
        switch (X) {
            ...
            default: 
                *r = 255; *g = 255; *b = 255; 
                //ESP_LOGW(TAG, "Poorly choosen max value ! f = %f", a); 
                break; 
        }
        r += 3;
        g += 3;
...
dl_matrix3du_t *rainbow_block(MotionEstContext *me_c, float max, int w, int h)
{
    dl_matrix3du_t *rgb = dl_matrix3du_alloc(1, w, h, 3);
    memset(rgb->item, 0, w*h*3);
    uint8_t *b = rgb->item;
    ...
    for(int i = 0; i < h; i++) {        
        const int x_mb = i >> me_c->log2_mbSize;
        const int wx_mb = x_mb * me_c->b_width;
        for(int j = 0; j < w; j++) {            
            const int y_mb = j >> me_c->log2_mbSize;
            const int mb_i = y_mb + wx_mb;
            const int val = me_c->mv_table[0][mb_i].mag2;
            const double a = (max > 0) ? 5.0 * val / max : 0;
            const int X = (int)a;
            const int Y = (int)255.0 * (a - X);
            switch (X) {
                case 7: *r = 255; *g = 0; *b = 0; break;
                case 6: *r = 255; *g = 0; *b = 0; break;
                case 5: *r = 255; *g = Y; *b = 0; break;
...
//MYCODE
    ...
    if (!image_matrix || !image_new || !vector) {
        ...
        if(!fmt2rgb888(fb->buf, fb->len, fb->format, image_matrix->item)){
            dl_matrix3du_free(image_matrix); free(vector);
        ...
        image_rainbow = rainbow(vector, max_mag2, w, h);
...
static esp_err_t stream_handler(httpd_req_t *req)
{
    ...
    dl_matrix3du_t *image_display = NULL;
    init = 1; 
    dl_matrix3du_t *black = dl_matrix3du_alloc(1, 240, 240, 3);
...
#if CONFIG_ESP_FACE_DETECT_ENABLED
            if ((!detection_enabled) && (motion_type < 2))
            {
#endif
                if(!init)
                    uninit(me_c); 
                init = 1;
                dl_matrix3du_free(image_motion); 
                image_motion = NULL; 
                me_c = NULL;
                if (fb->format != PIXFORMAT_JPEG){
                ...
                image_motion = NULL; 
                me_c = NULL;
                memset(black->item, 0, 172800);
                draw_boxes(black, 1, 1, mbsize + 1, mbsize + 1, FACE_COLOR_CYAN); 
                if(!rgb_printf(black, FACE_COLOR_CYAN, "MB: %u\np: %u", mbsize, search_parameter))
                    ESP_LOGW(TAG, "rgbprintf error!"); 
                if (!fmt2jpg(black->item, 240*240*3, 240, 240, PIXFORMAT_RGB888, 6, &_jpg_buf, &_jpg_buf_len))
                ...
                if(fb->format != PIXFORMAT_JPEG) {
                    ...
                    //update parameter
                    if(!init) {
                        me_c->search_param = search_parameter;
                        me_c->mbSize = mbsize;
                        me_c->method = motion_type;
                    }
                    ...
                    if (init) {    
                        ...
                        me_c->height = h;
                        //me_c->linesize = 1 * w;     
                        //allocate motion vect table
                        if(motion_type == BLOCK_MATCHING_EPZS || motion_type == BLOCK_MATCHING_ARPS) {
                            me_c->mbSize = mbsize;
                            me_c->search_param = search_parameter;
                        }
                        
                        if(!init_context(me_c))
                            ESP_LOGE(TAG, "couldn't init motion context");
                        ...
                        _jpg_buf_len = fb->len;
                        _jpg_buf = fb->buf;      
                    } else if(!motion_estimation(me_c, image_motion->item, image_matrix->item)) {
                        ...
                        arps_start = esp_timer_get_time(); 
                        
                        // setup image display to jpg output
                        int dispWidth  = w,
                            dispHeight = h;
                        ...
                        memcpy(image_motion->item, image_matrix->item, count);
                        esp_camera_fb_return(fb); fb = NULL;
                        ESP_LOGI(TAG, "display time: %u ms", (uint32_t)(esp_timer_get_time() - arps_start)/1000);
                        dl_matrix3du_free(image_display); image_display = NULL;
                    }
                }
    ...
    dl_matrix3du_free(image_motion); image_motion = NULL;
    dl_matrix3du_free(black); black = NULL;
    init = 1;
    return res;
}
